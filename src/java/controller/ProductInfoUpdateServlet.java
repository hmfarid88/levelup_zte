...
public class ProductInfoUpdateServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
                ...
                String query = "update stock set  MODEL=?, COLOR=?, IMEI=?, PURCHASE_PRICE=?, A_SELL_PRICE=?, B_SELL_PRICE=?, VENDOR=? where IMEI=?";
                ...
                ps.setString(8, himei);
                int a = ps.executeUpdate();
                if (a > 0) {
                    String upquery="update vendor_stock set PRODUCT=?, PRODUCT_ID=?, PURCHASE_PRICE=?, A_SALE_PRICE=?, B_SALE_PRICE=?, VENDOR=? where PRODUCT_ID=?";
