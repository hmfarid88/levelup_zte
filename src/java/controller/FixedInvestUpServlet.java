 ...
 * @author Acer
 */
public class FixedInvestUpServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       int investno=Integer.parseInt(request.getParameter("investno"));
       ...
       ResultSet rs=null;             
        try{
            ...
            String query="update fixed_invest set AMOUNT=AMOUNT+? where SI_NO=?";
            ps = con.prepareStatement(query);
            ps.setFloat(1, amount);
            ps.setInt(2, investno);
            int a = ps.executeUpdate();
             if (a > 0) {
            String query1="insert into cash_credit(CREDIT_NAME, AMOUNT, DATE) values(?,?, CURDATE())";
            ps = con.prepareStatement(query1);
             ps.setString(1, "Invest Plus");
             ps.setFloat(2, amount);
             ps.executeUpdate();
                response.sendRedirect("fixed_invest.jsp");
            } else {
