...
public class RtpayUpdateServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int rtpsi=Integer.parseInt(request.getParameter("rtpsi"));
        ...
        PreparedStatement ps = null;
         ...
         try{
            con = Database.getConnection();
                 ...
                 String payup="update customer_pay set AMOUNT=? where SI_NO=?";
                ps=con.prepareStatement(payup);
                ps.setFloat(1, amount);
                ps.setInt(2, rtpsi);
               int x= ps.executeUpdate();
               if(x>0){
                 String statemup="update retailer_statement set PAYMENT=? where PAYID=?";
                ps=con.prepareStatement(statemup);
                ps.setFloat(1, amount);
                ps.setInt(2, rtpsi);
                ps.executeUpdate();  
